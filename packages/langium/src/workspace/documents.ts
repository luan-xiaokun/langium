...
export interface LangiumDocumentFactory {
    ...
    /**
     * Create a Langium document for the given URI. The document content can be attached, and will
     * otherwise be fetched from the {@link TextDocuments} if present, or loaded via the {@link FileSystemProvider}.
     */
    create<T extends AstNode = AstNode>(uri: URI, content?: string | TextDocument): LangiumDocument<T>
...
export class DefaultLangiumDocumentFactory implements LangiumDocumentFactory {
    ...
    create<T extends AstNode = AstNode>(uri: URI, content?: string | TextDocument | { $model: T }): LangiumDocument<T> {
        ...
        // if still no document there try to load it from the file system
